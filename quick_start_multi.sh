...
run_create_nodes_env(){
    ...
    for i in {0001..0024}; do 
        ...
        mkdir -p $folder_name; 
        cp nodes/node000/config.ini $folder_name
        cp nodes/node000/genesis.json $folder_name
        #set_config
    done
...
set_config(){
    sed -i "s/producer-name = eosio/producer-name = node$i/" $folder_name/config.ini
